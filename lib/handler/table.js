...
function generateRows(table, rowValues) {
  var rows
    ...
    , tmpl;
  rows = table.find('.//*[name()="table:table-row"]');
  ...
  rowValues.forEach(function(values){
    var rowId = ++currentRow
      ...
      , cells;
    table.addChild(row);
    cells = row.find('.//*[name()="table:table-cell"]');
    ...
    cells.forEach(function(cell){
      ...
      cell
        .attr('style-name')
        .value( updateStyleName(styleName, rowId) );
    });
